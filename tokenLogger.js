...
async function logSession({ sesjonId, kundeNavn, jobbtype, skillVersjon, model, inputTokens, outputTokens, cacheReadTokens, cacheCreationTokens, toolCalls, notat }) {
  if (!TOKEN_LOG_DB) {
    console.warn('[tokenLogger] TOKEN_LOG_DB_ID ikke satt');
    return { logged: false, reason: 'no TOKEN_LOG_DB_ID' };
  }
...
async function getStats() {
  ...
  try {
    const response = await notion.databases.query({ database_id: TOKEN_LOG_DB, page_size: 100, sorts: [{ property: 'Dato', direction: 'descending' }] });
    const rows = response.results.map(p => ({
      sesjonId:            p.properties['Sesjon ID']?.title?.[0]?.plain_text || '',
      dato:                p.properties['Dato']?.date?.start || '',
      kunde:               p.properties['Kunde']?.rich_text?.[0]?.plain_text || '',
      ...
      outputTokens:        p.properties['Output tokens']?.number || 0,
    ...
    for (const [versjon, s] of Object.entries(byVersjon)) {
      ...
      summary[versjon] = {
        ...
        snittInputTokens:  Math.round(avg(s.map(x=>x.inputTokens))),
        snittCacheRead:    Math.round(avg(s.map(x=>x.cacheReadTokens))),
    ...
    const baseline = { antallSesjoner: 23, snittKostnad: 5.49, maksKostnad: 20.30, snittInputTokens: 2990, snittCacheRead: 13989885, snittCacheCreate: 527319, snittToolCalls: null, kilde: 'AI-playground proxy feb–apr 2026' };
...
module.exports = { logSession, getStats, estimateCost };
